...
from dsllm.data.utils import generate_chat_template, preprocess, preprocess_cls, get_token_list
...
class MultiChannelTimeSeriesDatasetStage2(Dataset):
    def __init__(self, data_path=None, qa_path=None, tokenizer=None, chronos_tokenizer=None, split=None, data_args=None):
        ...
        #     print("Dataset selected: usc-had")
        ...
        #     print("Dataset selected: mhealth")
        ...
        #     print("Dataset selected: pamap50")
        # elif dataset == 'pamap':
        #     self.SYS_INST = "The assistant is provided with time-series readings of 27 sensor channels, including acceleration sensors (in m/s^2) and gyroscope sensors (in rad/s) and magnetometer sensors (in μT). Each channel contains 100 data representing information extracted from the same 2-second time window at a sampling rate of 100Hz. Please analyze the trends and patterns in each channel to identify the correct activity type from the following twelve activity options:\n\n1. lying\n2. sitting\n3. standing\n4. walking\n5. running\n6. cycling\n7. Nordic walking\n8. ascending stairs\n9. descending stairs\n10. vacuum cleaning\n11. ironing\n12. rope jumping\n\nProvide the predicted activity as both the number and the name at the end."
        #     print("Dataset selected: pamap")
        # elif dataset == 'uci':
        ...
        #     print("Dataset selected: uci")
        # elif dataset == 'capture24':
        #     self.SYS_INST = "The assistant is provided with time-series readings of three accelerometer channels (in g). Each channel contains N data representing information extracted from the same time window. Please analyze the trends and patterns in each channel to identify the correct activity type from the following ten activity options:\n\n1. sleep\n2. sitting\n3. standing\n4. walking\n5. bicycling\n6. vehicle\n7. household-chores\n8. manual-work\n9. sports\n10. mixed-activity\n\nProvide the predicted activity as both the number and the name at the end."
        #     print("Dataset selected: capture24")
        # else:
        #     print(f"Wrong dataset name in __init__: {dataset}")
        #     raise ValueError(f"Wrong dataset name in __init__: {dataset}")
        # print(f"System instruction: {self.SYS_INST}")
        
        
        # Load config from YAML file
        config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../capture24_imp/config_stage2.yaml')
        ...
        # Overwrite with static Capture24 system instruction for all cases
        self.SYS_INST = (
            f"The assistant is provided with time-series readings of three accelerometer channels (in g). "
            f"Each channel contains {window_size_seconds * sampling_rate} data representing information extracted from the same {window_size_seconds}-second time window at a sampling rate of {sampling_rate}Hz. "
            f"Please analyze the trends and patterns in each channel to identify the correct activity type from the following ten activity options:\n\n"
            f"1. sleep\n2. sitting\n3. standing\n4. walking\n5. bicycling\n6. vehicle\n7. household-chores\n8. manual-work\n9. sports\n10. mixed-activity\n\n"
            f"Provide the predicted activity as both the number and the name at the end."
        )
        print(f"[FORCED] System instruction: {self.SYS_INST}")
        ...
        print(
            f"The dataset size is: {len(self.ts_data)}. Window size: {self.window_length}. Channel num: {self.channel_num}. Dataset: {dataset}\nSystem instruction: {self.SYS_INST}"
        )
...
class DataCollatorForTsTextDatasetStage2(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Batch input_texts and answers as lists
...
class MultiChannelTimeSeriesCLSDatasetStage2(Dataset):
    ...
    def _flatten_data(self, shuffle: bool, dataset: str):
        ...
        if self.split == 'train':
            ...
            # Ensure all classes from 0 to num_classes-1 are represented
            all_classes = range(len(self.label2id))
            value_counts = label_series.value_counts(normalize=True).reindex(all_classes).fillna(0)
            # Add a small epsilon to avoid division by zero
            value_counts = value_counts + 1e-6
            class_weights = (1 / value_counts).tolist()
            class_weights = torch.tensor(class_weights)
            class_weights = class_weights / class_weights.sum()
            print(f"Computed class weights for {len(self.label2id)} classes: {class_weights}")
        if shuffle:
            print("Shuffling data...")
            random.seed(RDM_SEED)
            indexes = list(range(len(ts_data)))
