...
class StanNormalizeUniformBins(ChronosTokenizer):
    def __init__(
            self, low_limit: float, high_limit: float, config: ChronosConfig
    ) -> None:
        self.config = config
        ...
        # print("self.centers.device:", self.centers.device)
        self.boundaries = torch.concat(
            (
                torch.tensor([-1e20], device=self.centers.device),
                (self.centers[1:] + self.centers[:-1]) / 2,
    ...
    def _input_transform(
            self, context: torch.Tensor, scale: Tuple[torch.Tensor, torch.Tensor] = None, eps: float = 1e-8
    ) -> Tuple[torch.Tensor, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        attention_mask = ~torch.isnan(context)

        if scale is None:
            mean = torch.nansum(context * attention_mask, dim=-1) / torch.nansum(attention_mask, dim=-1)
            variance = torch.nansum((context - mean.unsqueeze(dim=-1)) ** 2 * attention_mask, dim=-1) / torch.nansum(
                attention_mask, dim=-1)
            std = torch.sqrt(variance + eps)
            scale = (mean, std) # 添加 eps 以避免除以零

        scaled_context = (context - scale[0].unsqueeze(dim=-1)) / scale[1].unsqueeze(dim=-1)
        # print("scaled_context.device:", scaled_context.device)
        token_ids = (
                torch.bucketize(
                    input=scaled_context,
                    boundaries=self.boundaries,
    ...
    def label_input_transform(
            self, label: torch.Tensor, scale: Tuple[torch.Tensor, torch.Tensor]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        length = label.shape[-1]
    ...
    def output_transform(
            self, samples: torch.Tensor, scale: Tuple[torch.Tensor, torch.Tensor]
    ) -> torch.Tensor:
        mean_unsqueezed = scale[0].unsqueeze(-1).unsqueeze(-1)
        std_unsqueezed = scale[1].unsqueeze(-1).unsqueeze(-1)
        indices = torch.clamp(
            samples - self.config.n_special_tokens - 1,
            min=0,
            max=len(self.centers) - 1,
        )
        return self.centers[indices] * std_unsqueezed + mean_unsqueezed
...
class ChronosPipeline:
    ...
    @classmethod
    def from_pretrained(cls, *args, tc='MeanScaleUniformBins', **kwargs):
        """
        ...
        chronos_config.tokenizer_class = tc
