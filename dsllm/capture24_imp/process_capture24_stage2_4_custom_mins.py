...
print(f"\nStarting processing for {len(all_test_labels)} segments...")
...
error_count = 0
processed_count = 0
total_segments = len(all_test_labels)

processed_indices = set()

try:
    # Use tqdm for progress bar
    for idx, (l, d) in enumerate(tqdm(zip(all_test_labels, all_test_segments), total=total_segments)):
...
print("\nFinal file check:")
final_file = os.path.join(output_path, f"capture24_{split}_qa_stage2_cls.json")
if os.path.exists(final_file):
    print(f"Output file exists at: {final_file}")
    print(f"File size: {os.path.getsize(final_file) / (1024*1024):.2f} MB")
else:
    print(f"Warning: Output file not found at: {final_file}") 