...
from dsllm.train.sensorllm_trainer import SensorLLMTrainer, SensorLLMWeightedCELossTrainer
import logging as logger
from dsllm.data import make_ts_text_data_module, make_ts_text_data_module_stage2, make_ts_classification_data_module_stage2
import warnings
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_cache: bool = field(default=False, metadata={"help": "Whether to use caching in the model."})
...
def train():
    ...
    if not training_args.stage_2:
        # stage 1
        logger.warning("Using model of Stage 1 (With T5)")
        ...
        if model_args.model_type == "CasualLM":
            ...
            with open('../model/ts_backbone.yaml', 'r') as file:
    ...
    if not training_args.stage_2:
        logger.warning("Using data_module of Stage 1")
        data_module = make_ts_text_data_module(tokenizer=tokenizer, chronos_tokenizer=chronos_tokenizer, data_args=data_args)
    else:
        # * stage2
        if model_args.model_type == "CasualLM":
    ...
    if model_args.model_type == "CasualLM":
        ...
        def compute_metrics(eval_pred):
            ...
            # Convert single float values to arrays for single-class case
            if not isinstance(precision_per_class, (list, np.ndarray)):
                precision_per_class = [precision_per_class]
            if not isinstance(recall_per_class, (list, np.ndarray)):
                recall_per_class = [recall_per_class]
            if not isinstance(f1_per_class, (list, np.ndarray)):
