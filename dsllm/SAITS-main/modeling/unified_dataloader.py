...
class LoadDataForImputation(LoadDataset):
    ...
    def __init__(self, file_path, set_name, seq_len, feature_num, model_type):
        ...
        with h5py.File(self.file_path, "r") as hf:  # read data from h5 file
            grp = hf[set_name]
            # When imputing we want the *observed* (possibly sparse) series, not the ground truth.
            # In the datasets produced by make_saits_dataset.py the sparse signal is stored in "X_hat";
            # fall back to "X" for compatibility with original SAITS datasets.
            self.X = grp["X_hat"][:] if "X_hat" in grp else grp["X"][:]
